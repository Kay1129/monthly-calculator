...
//获取某月开销记录
router.get('/expense/:regDate', async (req, res) => {
    try {
        ...
        // 查询匹配记录，返回需要的字段
        const originalMonthlyDetaiListResult = await Expense.find(
            {regDate: regDate}, 
        ...
        const monthlyDetaiListResult = originalMonthlyDetaiListResult.map((item) => ({
            ...item._doc, // 获取原始数据对象
            price: "$" + item.price.toString(), // 将 `price` 转换为字符串
        }));
        

        res.status(200).json({regDate,totalPrice,monthlyDetaiListResult});
    } catch (err) {
        res.status(500).json({
            error: err.message
        });
