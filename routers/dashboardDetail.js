...
router.get('/expense/monthSummary/:regDate/', async (req, res) => {
    try {
        ...
        const averagePrice = parseFloat(totalPrice / availablePayerList.length).toFixed(2);
        ...
        const expenseSummary = completeExpenseByPayer.map(payer => {
            if (payer._id === "Yining") {
                ...
                .filter(otherPayer => otherPayer._id !== "Yining")
            ...
            else if(!availablePayerList.includes(payer._id)){
                return{
                    name: payer._id + " (currently On Leave)",
                    paid: "N/A",
                    toBePaid: "N/A",
                }
            }
