...
import { ChevronDownIcon } from '@heroicons/react/20/solid'
  ...
  export default function Example() {
    ...
    const [selectedOption, setSelectedOption] = useState("Lenora"); // 默认选中值
    const options = ["Lenora", "Christy", "Kai", "Yining", "Tang"]; // 可选项
    ...
    useEffect(() => {
        ...
        //test dbDate
        const testDbDate = "2025-October"
        // 定义一个异步函数以发送 GET 请求
        const fetchData1 = async () => {
            try {
            const response = await fetch('http://localhost:3000/api/expense/'+ dbDate); // 替换为你的 API 地址
            if (!response.ok) {
